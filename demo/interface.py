...
welcome_message = """
  ...
  Hint: The knowledge base of the LLM has been loaded with a pre-existing vectorstore of [resumes](https://github.com/Gaurravvvv/Resume-Screening-RAG-Pipeline/blob/main/data/main-data/synthetic-resumes.csv) to be used right away. 
  In addition, you may also find example job descriptions to test [here](https://github.com/Gaurravvvv/Resume-Screening-RAG-Pipeline/blob/main/data/supplementary-data/job_title_des.csv).
...
about_message = """
  ...
  The program is very much a work in progress. I really appreciate any contribution or feedback on [GitHub](https://github.com/Gaurravvvv/Resume-Screening-RAG-Pipeline).
      ...
      if "Resume" not in df_load.columns or "ID" not in df_load.columns:
        ...
        with st.toast('Indexing the uploaded data. This may take a while...'):
          try:
            ...
            with modal.container():
              st.markdown("An error occurred during indexing. Please check your data format.")
              st.error(error)
  else:
    st.session_state.df = pd.read_csv(DATA_PATH)
    vectordb = FAISS.load_local(FAISS_PATH, st.session_state.embedding_model, distance_strategy=DistanceStrategy.COSINE, allow_dangerous_deserialization=True)
...
def check_openai_api_key(api_key: str):
  ...
  if api_key in st.session_state.api_key_valid:
    return st.session_state.api_key_valid[api_key]
  ...
  try:
    ...
    st.session_state.api_key_valid[api_key] = "valid"
    return "valid"
  except openai.AuthenticationError as e:
    st.session_state.api_key_valid[api_key] = "invalid"
    return "invalid"
  except openai.RateLimitError as e:
    if "insufficient_quota" in str(e):
      st.session_state.api_key_valid[api_key] = "quota_exceeded"
      return "quota_exceeded"
    st.session_state.api_key_valid[api_key] = "rate_limited"
    return "rate_limited"
  except Exception:
...
def check_model_name(model_name: str, api_key: str):
  if "model_valid" not in st.session_state:
    st.session_state.model_valid = {}
  cache_key = (model_name, api_key)
  if cache_key in st.session_state.model_valid:
    return st.session_state.model_valid[cache_key]

  openai.api_key = api_key
  try:
    model_list = [model.id for model in openai.models.list()]
    valid = model_name in model_list
    st.session_state.model_valid[cache_key] = valid
    return valid
  except Exception:
    return False
...
with st.sidebar:
  ...
  st.markdown("Made by [Gaurravvvv](https://github.com/Gaurravvvv)")
...
api_key_status = check_openai_api_key(st.session_state.api_key)
if api_key_status == "invalid":
  ...
  st.stop()
elif api_key_status == "quota_exceeded":
  st.error("You have exceeded your OpenAI API quota (insufficient credit balance). Please check your plan and billing details at [OpenAI Usage](https://platform.openai.com/usage).")
  st.stop()
elif api_key_status == "rate_limited":
  st.error("OpenAI API is currently rate limited. Please try again in a moment.")
  st.stop()
elif api_key_status == "connection_error":
  st.warning("Failed to connect to OpenAI API. Please check your internet connection or API settings.")
  st.stop()
...
if user_query is not None and user_query != "":
  ...
  # Format history for LLM context (excluding verbosity metadata tuples)
  formatted_history = []
  ...
  with st.chat_message("AI"):
    ...
    with st.spinner("Generating answers..."):
      ...
      if query_type == "no_retrieve":
        ...
        stream_message = stream_string(response_text)
        document_list = []
      else:
        st.session_state.resume_list = document_list
        stream_message = llm.generate_message_stream(user_query, document_list, formatted_history, query_type)
